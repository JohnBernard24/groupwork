...
*/
	...
	function celsiusToFahrenheit(temperatures){
		let convertedArray = temperatures.map(function(temperature){
...
*/
    function hasDivisibleBy8(numArr) {
        let hasDivisible = numArr.some(function(num){
            return (num % 8 === 0);
        });
...
*/
    function filterEvenNumbers(numArr) {
        let filteredNum = numArr.filter(function(num){
            return (num % 2 === 0);
        });
...
function getProductNames(productsArr) {
        let productNames = productsArr.map(function(product){
            return product.name;
        });
